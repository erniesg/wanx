    ...
    if not pexels_api_key:
        ...
        if script2_data and "script_segments" in script2_data:
            ...
            if hook_keywords:
                ...
                query1 = " ".join(hook_keywords)
                print(f"\nSearching for 1 video with query: '{query1}'")
                downloaded = find_and_download_videos(pexels_api_key, query1, 1, test_output_dir, orientation="portrait")
                if downloaded:
                    print(f"Downloaded files: {downloaded}")
                else:
                    print("No videos downloaded for the first query.")
            else:
            ...
            if body_keywords:
                 ...
                 query2 = " ".join(body_keywords[:3]) # Use first 3 keywords
                 print(f"\nSearching for 1 video with query: '{query2}'")
                 downloaded = find_and_download_videos(pexels_api_key, query2, 1, test_output_dir, orientation="portrait")
                 if downloaded:
                     print(f"Downloaded files: {downloaded}")
                 else:
                     print("No videos downloaded for the second query.")
            else:
        ...
        if script4_data and "script_segments" in script4_data:
            ...
            if conflict_keywords:
                 ...
                 print(f"\nSearching for 1 video with query: '{query3}'")
